...
- (void)excuteMsg:(NSString *)messageQueueString moduleName:(NSString *)moduleName
{
    ...
    if (bs == nil) {
        ...
        NSLog(@"模块未注册");
        NSString *callbackId = message[@"callbackId"];
        [self callbackWithId:callbackId code:@0 msg:@"模块未注册"];
        return;
    }
...
//        continue;
    ...
    if (responseId) {
        ...
        //            WVJBHandler handler = self.messageHandlers[message[@"handlerName"]];
        WVJBHandler handler = [bs handler:message[@"handlerName"]];
...
- (void)excuteMsg:(NSDictionary *)msgDic {
    ...
    /*
     权限判断
     */
    NSMutableDictionary *accessDic = self.modulesAuthorityDic[moduleName];
    if (!accessDic) {
        NSString *msg = [NSString stringWithFormat:@"%@:该模块没有权限",moduleName];
        [self callbackWithId:callbackId code:@0 msg:msg];
        return;
    } else {
        int access = [[accessDic objectForKey:handlerName] intValue];
        if (access <= 0) {
            NSString *msg = [NSString stringWithFormat:@"%@:该方法没有权限",handlerName];
            [self callbackWithId:callbackId code:@0 msg:msg];
            return;
        }
    }
...
- (NSMutableDictionary *)modulesAuthorityDic {
    if (_modulesAuthorityDic == nil) {
        _modulesAuthorityDic = [NSMutableDictionary dictionary];
    }
    return _modulesAuthorityDic;
}
